...
source_url: "https://developers.google.com/search/docs/appearance/title-link?hl=en"
product: "Google Search Central"
section: "Docs / Appearance & Structured Data"
language: en
scraped_date: 2026-06-08
doc_id: "search-central/docs/appearance/title-link.md"
---
...
[How to make crispy fried eggs](https://wikipedia.org/wiki/Fried_egg)
...
* Avoid **keyword stuffing**. It's sometimes helpful to have a few descriptive
  ...
  and this kind of [keyword stuffing](/search/docs/essentials/spam-policies#keyword-stuffing) can make your results look spammy to Google and to users.
* Avoid **repeated or boilerplate text in `<title>` elements**. It's
  ...
  One solution is to dynamically update the `<title>` element to better reflect the actual content of the
...
* **Brand your titles** concisely. The `<title>` element on your site's home page is
  ...
  But displaying that text in the `<title>` element of every single page on
  your site will look repetitive if several pages from your site are returned for the same query. In
  this case, consider including just your [site name](/search/docs/appearance/site-names)
  at the beginning or end of each `<title>` element,
...
* **Make it clear which text is the main title** for the page. Google looks at
  various [sources when creating title links](#sources), including the main visual title, heading elements,
...
* **Be careful about disallowing search engines** from crawling your pages.
  Using the [robots.txt](/search/reference/robots_txt) protocol on your site can
  ...
  can use the [`noindex`
  rule](/search/docs/crawling-indexing/block-indexing).
...
* **Avoid including flight price information in `<title>`
  elements.** Our systems will likely not show price information when generating title
...
* [`WebSite` structured data](/search/docs/appearance/site-names#how-to-add-structured-data)
...
`<title>` element or main heading**: If we've detected an
[issue on the page](#issues), we may try to generate an improved title link from
anchors, on-page text, or other [sources](#sources).
...
| Duplication of the [site name](/search/docs/appearance/site-names) in the `<title>` element | In the case of domain-level site names, Google may omit the site name from the title link, if it's repetitive with the [site name that's already shown in the search result](/search/docs/appearance/site-names). |
