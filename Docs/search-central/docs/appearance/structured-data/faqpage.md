...
source_url: "https://developers.google.com/search/docs/appearance/structured-data/faqpage?hl=en"
product: "Google Search Central"
section: "Docs / Appearance & Structured Data"
language: en
scraped_date: 2026-06-08
doc_id: "search-central/docs/appearance/structured-data/faqpage.md"
---
...
![FAQ rich result in Google Search](/static/search/docs/images/faqpage-searchresult.png)
...
[`QAPage`](/search/docs/appearance/structured-data/qapage) structured data instead.
...
* Only use `FAQPage` if your page contains FAQs where there's a single answer to each question. If your page
  has a single question and users can submit alternative answers, use
  [`QAPage`](/search/docs/appearance/structured-data/qapage) instead. Here are some examples:
...
* All `FAQ` content must be visible to the user on the source page. Here are some examples:
  ...
  **Valid use cases**:
  ...
  + The question is visible on the page, and the answer is hidden behind an expandable section. The user is
    able to access the answer by clicking the expandable section.
...
| Required properties | |
| --- | --- |
| `mainEntity` | `Question` An array of `Question` elements which contain the list of answered questions that this `FAQPage` is about. You must specify at least one valid [`Question` item](#question). A `Question` item includes both the question and answer. |
...
| Required properties | |
| --- | --- |
| `text` | `Text` The full answer to the question. The answer may contain HTML content such as links and lists. Google Search displays the following HTML tags: `<h1>` through `<h6>`, `<br>`, `<ol>`, `<ul>`, `<li>`, `<a>`, `<p>`, `<div>`, `<b>`, `<strong>`, `<i>`, and `<em>`. All other tags are ignored. |
