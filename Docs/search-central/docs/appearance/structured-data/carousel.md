...
source_url: "https://developers.google.com/search/docs/appearance/structured-data/carousel?hl=en"
product: "Google Search Central"
section: "Docs / Appearance & Structured Data"
language: en
scraped_date: 2026-06-08
doc_id: "search-central/docs/appearance/structured-data/carousel.md"
---
...
1. Decide which page will contain the carousel structured data. There are two options:
   * **[Summary page and multiple detail pages](#summary)**: The summary page has a short
   ...
   * **[A single, all-in-one-page list](#all-in-one)**: A single page that contains all
     list information, including full text of each item. For example, a list of the top movies in
     2020, all contained on one page.
2. Add the [required properties](#structured-data-type-definitions). Based on the
...
3. Add the required and recommended properties for the specific content type that the carousel is about:
   * [Course](/search/docs/appearance/structured-data/course)
   * [Movie](/search/docs/appearance/structured-data/movie)
   * [Recipe](/search/docs/appearance/structured-data/recipe)
   * [Restaurant](/search/docs/appearance/structured-data/local-business#carousel)
...
1. Validate your code using the
   [Rich Results Test](https://search.google.com/test/rich-results).
   For a [summary page](#summary), there are some things you need to verify yourself:
   ...
   * Validate each URL that's mentioned in the list using the Rich Results Test. Each page in the list
     ...
     [Recipe](/search/docs/appearance/structured-data/recipe),
     [Course](/search/docs/appearance/structured-data/course),
     [Restaurant](/search/docs/appearance/structured-data/local-business#carousel-example),
     [Movie](/search/docs/appearance/structured-data/movie).
...
* If this is a [summary page](#summary), include only
...
| Required properties | |
| --- | --- |
| `item` | `Thing`  An individual thing in a list. Populate this object with the following values, plus all the properties of the specific structured data type being described:   * `item.name` * `item.url` * Any other properties required for this data type, as described in schema.org and the rules   described in the documents for your content type:   + [Course](/search/docs/appearance/structured-data/course)   + [Movie](/search/docs/appearance/structured-data/movie)   + [Recipe](/search/docs/appearance/structured-data/recipe)   + [Restaurant](/search/docs/appearance/structured-data/local-business#carousel)**Example**: For a recipe, you would provide `prepTime` and `image`   properties. |
