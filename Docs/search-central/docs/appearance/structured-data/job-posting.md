...
source_url: "https://developers.google.com/search/docs/appearance/structured-data/job-posting?hl=en"
product: "Google Search Central"
section: "Docs / Appearance & Structured Data"
language: en
scraped_date: 2026-06-08
doc_id: "search-central/docs/appearance/structured-data/job-posting.md"
---
...
![An illustration of how the job posting experience can appear in Google Search results](/static/search/docs/images/jobs-search-ui.png)
...
1. Ensure that Googlebot can
   [crawl your site efficiently](/search/docs/crawling-indexing/troubleshoot-crawling-errors#improve_crawl_efficiency).
2. If you have several copies of the same job posting on your site hosted under different
   URLs, [use canonical URLs](/search/docs/crawling-indexing/consolidate-duplicate-urls)
   on each copy of the page.
3. Add the [required and recommended properties](#structured-data-type-definitions). Based on the
...
7. Keep Google informed by using the Indexing API and submitting a sitemap.
   ...
   sitemaps because the Indexing API prompts Googlebot to crawl your page sooner. Use the [Indexing API](/search/apis/indexing-api) to notify Google of a
   new URL to crawl or that content at a URL has been updated.

   However, we still recommend [submitting a sitemap](/search/docs/crawling-indexing/sitemaps/overview)
   for coverage of your entire site. We ingest the entire sitemap and recrawl the pages with `lastmod`
...
the following ways. Failure to take timely action on expired jobs may result in a [manual action](https://support.google.com/webmasters/answer/2604824).
...
2. Keep Google informed of changes:
   ...
   [submitting a sitemap](/search/docs/crawling-indexing/sitemaps/overview)
   for coverage of your entire site.
   * Use the [Indexing API](/search/apis/indexing-api) to request removal of
     a job posting URL from the Google Search index.
...
![Work from home job posting example in Search results](/static/search/docs/images/job-work-from-home.png)
...
`hiringOrganization` logo. The `logo`
...
* If you choose to use sitemaps to keep Google informed of changes to job posting URLs,
  follow the [general sitemap guidelines](/search/docs/crawling-indexing/sitemaps/build-sitemap#general-guidelines).
  ...
  + The URLs in the sitemap must include
    [the canonical page](/search/docs/crawling-indexing/consolidate-duplicate-urls)
...
* Jobs posting for fake or non-existent jobs. This includes job postings with the primary
  ...
  applicants.
* Job titles, description, and other details that use [keyword stuffing](/search/docs/essentials/spam-policies#keyword-stuffing)
...
| Required properties | |
| --- | --- |
| `datePosted` | `Date`  The original date that employer posted the job in [ISO 8601 format](https://en.wikipedia.org/wiki/ISO_8601). For example, "2017-01-24" or "2017-01-24T19:33:17+00:00".     ``` "datePosted": "2016-02-18" ``` |
...
| `baseSalary` | `MonetaryAmount`  The actual base salary for the job, as provided by the employer (not an estimate). **Note**: Only employers can provide `baseSalary`.  For the `unitText` of `QuantitativeValue`, use one of the following case-sensitive values:   * `HOUR` * `DAY` * `WEEK` * `MONTH` * `YEAR`   For example:     ``` "baseSalary": {   "@type": "MonetaryAmount",   "currency": "USD",   "value": {     "@type": "QuantitativeValue",     "value": 40.00,     "unitText": "HOUR"   } } ```   To specify a salary range, define a `minValue` and a `maxValue`, rather than a single `value`. For example:     ``` "baseSalary": {   "@type": "MonetaryAmount",   "currency": "USD",   "value": {     "@type": "QuantitativeValue",     "minValue": 40.00,     "maxValue": 50.00,     "unitText": "HOUR"   } } ``` |
...
| `validThrough` | `DateTime` **Note**: This is required for job postings that have an expiration date. The date when the job posting will expire in [ISO 8601 format](https://en.wikipedia.org/wiki/ISO_8601). For example, "2017-02-24" or "2017-02-24T19:33:17+00:00". For example:     ``` "validThrough": "2017-03-18T00:00" ```   If a job posting never expires, or you do not know when the job will expire, do not include this property. If the job is filled before the expiration date occurs, [remove the job posting](#remove). |
...
1. Remove `JobPosting` structured data from the listing page. Only place
   `JobPosting` structured data on pages that are dedicated to a single job posting.
2. After you resolve the issue,
...
2. Use the [URL Inspection tool](https://support.google.com/webmasters/answer/9012289)
   to make sure that the content is visible on the rendered page (the rendered page is how
   Google sees your page).
3. After you resolve the issue,
...
* **Make sure you specify the correct logo with the
  `hiringOrganization.logo` property.**
  Make sure that the image width and height ratio is between 0.75 and 2.5.
...
1. Make sure the structured data includes values for `jobLocation`,
