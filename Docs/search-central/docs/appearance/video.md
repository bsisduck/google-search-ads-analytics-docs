...
source_url: "https://developers.google.com/search/docs/appearance/video?hl=en"
product: "Google Search Central"
section: "Docs / Appearance & Structured Data"
language: en
scraped_date: 2026-06-08
doc_id: "search-central/docs/appearance/video.md"
---
...
* Don't use [fragment identifiers](https://wikipedia.org/wiki/URI_fragment)
...
* If you're using JavaScript to inject the video, make sure that it
  [appears in the rendered HTML in the URL inspection tool](/search/docs/crawling-indexing/javascript/javascript-seo-basics).
...
* The video can't be hidden behind other elements.
  ...
  before seeing the video), add [paywall structured data](/search/docs/appearance/structured-data/paywalled-content)
  so that Google can still find and index the video.
* The video must have a [valid thumbnail](#valid-thumbnail) that's available at a
...
![a web page where the video is the main content on the page](/static/search/docs/images/dedicated-video-page.png)
...
still be eligible to appear as a [text result](/search/docs/appearance/visual-elements-gallery#text-result)
and [Google Image result with a video badge](/search/docs/appearance/structured-data/video).
...
![URLs on a video page](/static/search/docs/images/diagram-urls-in-page.png)
...
| **Location** | The thumbnail file must be accessible by Googlebot and Googlebot Images (don't block the file with [robots.txt](/search/docs/crawling-indexing/robots/intro) or a login requirement). Make sure that the file is consistently available at a [stable URL](#stable-url). |
...
in our [general structured data guidelines](/search/docs/appearance/structured-data/sd-policies).

### Video previews

![Video preview in search results](/static/search/docs/images/video-preview.gif)
...
* **If your video is embedded on your web page or you're running a video platform**, there are two ways that you can enable key moments:
  + [`Clip` structured data](/search/docs/appearance/structured-data/video#clip):
  ...
  + [`SeekToAction` structured data](/search/docs/appearance/structured-data/video#seek):
...
* **If your video is hosted on YouTube**, you can specify the exact timestamps and labels
  ...
  [best practices for marking timestamps in YouTube descriptions](/search/docs/appearance/structured-data/video#best-practices-youtube).
...
[`nosnippet`](/search/docs/appearance/snippet#nosnippet) `meta` tag.
...
[really is Googlebot](/search/docs/crawling-indexing/verifying-googlebot).
...
* Allow Google to fetch the video's streaming file URL (such as M3U8). Don't block the URL of
  the actual video bytes with the [`noindex`](/search/docs/crawling-indexing/block-indexing)
  rule or [robots.txt](/search/docs/crawling-indexing/robots/intro) file.
* The video file must be available at a [stable URL](#stable-url).
* Use structured data to provide the [`contentURL`](/search/docs/appearance/structured-data/video#content-url)
  value of a [supported file type](#supported-video-files).
* The host of the video watch page and the server that's streaming the actual video must have
  ...
  [technical requirements for Google Search](/search/docs/essentials/technical) and
  have [available server capacity](/search/docs/crawling-indexing/troubleshoot-crawling-errors#availability_issues).
...
[file a removal request](/search/docs/crawling-indexing/remove-information) for the
...
* **Include a [`noindex`](/search/docs/crawling-indexing/robots-meta-tag)
...
* **Indicate an expiration date** in your structured data (the
  [`expires`](/search/docs/appearance/structured-data/video#expires)
...
[`regionsAllowed`](/search/docs/appearance/structured-data/video#regions-allowed)
