...
source_url: "https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap?hl=en"
product: "Google Search Central"
section: "Docs / Crawling & Indexing"
language: en
scraped_date: 2026-06-08
doc_id: "search-central/docs/crawling-indexing/sitemaps/build-sitemap.md"
---
...
sitemaps,
[read our introduction first](/search/docs/crawling-indexing/sitemaps/overview).
...
create a [sitemap index](/search/docs/crawling-indexing/sitemaps/large-sitemaps)
...
unless you submit your sitemap through [Search Console](#addsitemap), a sitemap
...
generally shows the
[canonical URLs](/search/docs/crawling-indexing/consolidate-duplicate-urls) in its
...
[annotate](/search/docs/crawling-indexing/mobile/mobile-sites-mobile-first-indexing#additional-best-practices)
...
[images](/search/docs/crawling-indexing/sitemaps/image-sitemaps),
[video](/search/docs/crawling-indexing/sitemaps/video-sitemaps), and
[news](/search/docs/crawling-indexing/sitemaps/news-sitemap) content, as well as
the [localized versions](/search/docs/specialty/international/localized-versions)
of your pages.
...
* You can use an
  [mRSS (media RSS) feed](/search/docs/crawling-indexing/sitemaps/video-sitemaps)
...
[canonical URLs](/search/docs/crawling-indexing/consolidate-duplicate-urls). If you
...
* For sitemaps with less than a few dozen URLs,
  you can [manually create a sitemap](#manualsitemap).
* For sitemaps with more than a few dozen URLs,
  [automatically generate a sitemap](#autositemap).
...
and follow a syntax described in the [Sitemap Formats](#sitemapformat) section. You
can name the file anything you like as long as
[the characters are allowed in a URL](/maps/url-encoding).
...
Google. Note the
[size requirements for sitemaps](#general-guidelines); if the sitemap becomes too
large, you must split it into smaller sitemaps. Learn more about
[managing large sitemaps](/search/docs/crawling-indexing/sitemaps/large-sitemaps).
...
* **Use the Search Console API** to programmatically
  [submit a sitemap](/webmaster-tools/v1/sitemaps/submit).
...
2. [Create a sitemap](#createsitemap) (or more if you prefer) that includes URLs
   from all the sites that you want to cover. You can include the sitemaps in a
   [sitemap index](/search/docs/crawling-indexing/sitemaps/large-sitemaps) file if
...
1. [Create one or more sitemaps](#createsitemap) for each individual site. For each
