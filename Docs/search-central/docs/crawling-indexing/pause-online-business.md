...
source_url: "https://developers.google.com/search/docs/crawling-indexing/pause-online-business?hl=en"
product: "Google Search Central"
section: "Docs / Crawling & Indexing"
language: en
scraped_date: 2026-06-08
doc_id: "search-central/docs/crawling-indexing/pause-online-business.md"
---
...
* **Display a banner or popup**: A banner or popup div on all pages including the
  ...
  use the [`data-nosnippet` HTML attribute](/search/docs/crawling-indexing/robots-meta-tag#data-nosnippet-attr).
...
* **Update your structured data**: If your site uses structured data (for example,
  ...
  [`Event`](/search/docs/appearance/structured-data/event)), make sure to adjust it appropriately
...
* **Tell Google about your updates**: To ask Google to recrawl a
  limited number of pages (for example, the home page), use [Search Console](/search/docs/crawling-indexing/ask-google-to-recrawl). For a larger number of pages (for example, all of your product
  pages), [use sitemaps](/search/docs/guides/submit-URLs).

Not recommended: Disable the whole website
------------------------------------------

**Warning**: Google's systems are designed to be robust and
...
* If you need to urgently disable the site for 1-2 days, then return an informational
  ...
  HTTP response status code](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/503) instead of all content. Make sure to follow the
  [best practices for disabling a site](#best-practices-disabling-site).
...
website if a page returns a `503` HTTP response status code.
...
* Confirm a `503` HTTP response status code locally by using
  [curl](https://curl.haxx.se/docs/manpage.html) or a similar tool. For example:
