...
source_url: "https://developers.google.com/search/docs/crawling-indexing/javascript/javascript-seo-basics?hl=en"
product: "Google Search Central"
section: "Docs / Crawling & Indexing"
language: en
scraped_date: 2026-06-08
doc_id: "search-central/docs/crawling-indexing/javascript/javascript-seo-basics.md"
---
...
Learn how to fix JavaScript-related problems with our [troubleshooting guide](/search/docs/guides/fix-search-javascript).
...
![Googlebot takes a URL from the crawl queue,
    ...
    and extracts links to put them into the crawl queue.](/static/search/docs/images/googlebot-crawl-render-index.png)
...
Googlebot queues all pages with a `200` HTTP status code for rendering, unless a [robots `meta` tag or header](/search/docs/crawling-indexing/robots-meta-tag) tells Google not to index the page.
...
Unique, descriptive [`<title>` elements](/search/docs/appearance/title-link#page-titles) and [meta descriptions](/search/docs/appearance/snippet#meta-descriptions) help users quickly identify the best result for their goal.
...
Conflicting or multiple `rel="canonical"` link tags may lead to unexpected results.
...
`meta` tag from `noindex` may not work as expected. If you *do* want the page indexed, don't use a
