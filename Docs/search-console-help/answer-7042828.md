...
source_url: "https://support.google.com/webmasters/answer/7042828?hl=en"
product: "Google Search Console"
section: "Help Center"
language: en
scraped_date: 2026-06-08
doc_id: "search-console-help/answer-7042828.md"
---
# What are impressions, position, and clicks?

This page helps explain impressions, position values, and click data as used in Search Console, and especially in the [Performance reports](https://www.google.com/webmasters/tools/search-analytics).
...
- [What is an impression?](https://support.google.com/webmasters/answer/7042828?hl=en#impressions)
- [What is search result position?](https://support.google.com/webmasters/answer/7042828?hl=en#position)
- [What is a click?](https://support.google.com/webmasters/answer/7042828?hl=en#click)
- [What URL is my data assigned to?](https://support.google.com/webmasters/answer/7042828?hl=en#url)
- [Additional methodology details by result type](https://support.google.com/webmasters/answer/7042828?hl=en#element-details)

## Introduction

Search Console provides data showing how often users saw or interacted with links to or content from your site, in Google Search, News, and Discover. This data is available in the various [performance reports](https://support.google.com/webmasters/topic/9384513).

The performance reports show the following metrics:

- **Impressions**: How often someone saw a link to your site on Google. Depending on the result type, the link might need to be scrolled or expanded into view. [See more details below.](https://support.google.com/webmasters/answer/7042828?hl=en#impressions)
- **Clicks**: How often someone clicked a link from Google to your site. [See more details below.](https://support.google.com/webmasters/answer/7042828?hl=en#click)
- **(average) Position**: A relative ranking of the position of your link on Google, where 1 is the topmost position, 2 is the next position, and so on. Shown only for Google Search results. [See more details below.](https://support.google.com/webmasters/answer/7042828?hl=en#position)
...
Content can be displayed in [many different formats](https://developers.google.com/search/docs/advanced/appearance/search-result-features) in Google Search, including links, images, or snippets of information. For example, here is a very basic search result that includes only one link (the classic " [plain blue link](https://developers.google.com/search/docs/advanced/appearance/search-result-features#general-categories-of-search-results)"):

![Basic search result with one link. And yes, this is the intended spelling so don't file a bug :)](https://lh3.googleusercontent.com/R50R-W-jCzkkCj1bgAw4cUj11jEkU2N9cYtnj3kxKW2U811czjvmXiChyZw6wUR7_A=w512)

An element can also be a compound element that contains many links, and even interactive elements. For example, a [horizontally scrolling list](https://developers.google.com/search/docs/data-types/carousel) of AMP pages, or a [knowledge panel entry](https://developers.google.com/search/docs/advanced/appearance/search-result-features#expandable-4) with many links.

All data is assigned to links in the element (or rather, to the URL that each link points to). Click, impression, and position data are attributed to the _canonical_ URL of the link. A canonical URL is basically the URL that Google chooses as the URL that best represents a page, when multiple URLs point to what is essentially the same page (for example, if a site has separate URLs for the mobile and desktop versions of a page). [See more details](https://support.google.com/webmasters/answer/7042828?hl=en#url).
...
However, inside independently scrolling or expanding search result widgets, such a [results carousel](https://developers.google.com/search/docs/data-types/carousel) or an expanding section like an [FAQ](https://support.google.com/webmasters/answer/7042828?hl=en#faq) result, the item must typically be scrolled into view within the carousel, or expanded by a click, to register an impression.

For scrolled results without paging ( _infinite scrolling_), such as image search in mobile or a card in Discover, the item must typically be scrolled into view to count an impression.
...
![Knowledge graph card showing links ](https://lh3.googleusercontent.com/diLtO6bHkcWSmAdBFdfTG0bzOyIgowXAZvH_s0NWONp6aXeOpMXRwLgY0rRad_-De30=w700)

- If you look at the Performance report with data **grouped by property**, only one impression is counted for the entire card:

  - www.example.com - _1 impression_
- However, if you look at the report with data **grouped by page**, you would see five pages with one impression each:

  - www.example.com (text link) - _1 impression_
  - www.example.com/rain (one text link, one image link) - _1 impression_
  - www.example.com/rainbow (one text link, one image link) - _1 impression_
  - www.example.com/shamrock (one image link) - _1 impression_
  - www.example.com/lightning (one image link) - _1 impression_
...
See [element-specific details](https://support.google.com/webmasters/answer/7042828?hl=en#element-details) for additional information about what counts as an impression.
...
Position value is the _average_ position for _all_ searches. For your specific search your position might be different than the average because of many variables, such as your search history, location, and so on.

#### Calculating the position

Google Search pages are very complex, and trying to determine a position for any given element is not very straightforward. In Search results, only elements containing at least one [non-query-refinement](https://support.google.com/webmasters/answer/7042828?hl=en#query-refinement) link occupy a position. Elements that have no links, or have only query refinement links, do not occupy a position. For example, a [carousel of van Gogh paintings](https://www.google.com/search?q=van+gogh+paintings&oq=van+gogh+paintings), in which each image opens a "query refinement" (a new Google search) is not counted as a position placeholder, and the carousel presence does not affect the position value of elements below it on the page. If a non-positional carousel had appeared in the example above above the AMP page carousel (at position 2), it would not affect the position values anywhere on the page.
...
Search Console simplifies analyzing your performance data by choosing one [_canonical URL_](https://developers.google.com/search/docs/advanced/crawling/consolidate-duplicate-urls#definition) that represents all variations of a page. These variants include variations by device (desktop or mobile), variant URLs that point to the same page, and possibly even alternate language versions of a page.

Click, impression, and position data for all variations of a page are assigned to the canonical URL that Google selects for each page (although in some cases data might be assigned to the actual URL, rather than the canonical URL). This means that even if you have separate URLs for the mobile and desktop version of a page, all click data will be assigned to the same URL in the Performance report. This way, you don't have to manually add all data for your mobile URLs to the equivalent desktop URLs to see how your page is performing. (Note that you can add a [filter](https://support.google.com/webmasters/answer/7576553#filteringdata) in the report to split your data by device or other category, if you choose).
...
[AI Mode](https://support.google.com/websearch/answer/16011537) expands on AI Overviews to show a more interactive AI-powered response with links to web resources that support the information or direct the user to view relevant webpages. AI Mode groups the user's question into subtopics and searches for each one simultaneously, and users can go deeper.

- **Click**: Clicking a link to an external page in AI Mode counts as a click.
- **Impression**: [Standard impression rules apply](https://support.google.com/webmasters/answer/7042828#impressions).
- **Position**: Position in AI Mode follows the [same methodology as a Google Search results page](https://support.google.com/webmasters/answer/7042828#position). Generally, carousel and image blocks within AI Mode are calculated using the standard position rules for those elements.
...
**Note**: Search Console doesn't include data from [experiments in Search Labs](https://support.google.com/websearch/answer/14184960), as these experiments are still in active development.

## AI Overviews

[AI Overviews](https://developers.google.com/search/docs/appearance/ai-overviews) show an overview with links to web resources that support the information.
...
- **URL:** The URL associated with an app install link is the root URI of the device (without the app://< _package\_or\_id_ \> prefix), which is "/".

## Carousel [rich result](https://support.google.com/webmasters/answer/7506797)

[Carousel](https://developers.google.com/search/docs/guides/mark-up-listings) rich results are scrolling containers (typically horizontally scrolling) that contain a set of items of the same type, such as image thumbnails or AMP pages.
...
[Discover](https://support.google.com/webmasters/answer/9046777) is a scrollable list of topic cards that users can browse on their mobile devices. Discover content is shown to users based on their interests. Each topic card includes a title, a few lines of text, and an image from the source page. Users tap the card to visit the source page.
...
A [featured snippet](https://support.google.com/webmasters/answer/6229325) displays information extracted from a single, specific web page. It includes a link to the source web page.
...
[Rich results are defined using structured data](https://developers.google.com/search/docs/advanced/appearance/search-result-features?hl=en#rich-result) and can be displayed in a variety of ways in Google Search results. The following general rules apply to all rich results that aren't described elsewhere on this page.
...
Image results can appear as embedded thumbnails in the [combined search results page](https://www.google.com/search?q=daffodils) or inline in the [image search results page](https://www.google.com/search?q=daffodils&tbm=isch). In the combined search results page they sometimes appear in a carousel of images.
...
![Counting order for position in image search results, in a left-to-right result set](https://lh3.googleusercontent.com/n8j4vk4Pryce7l7FJmDMG88olYfNNjXrUSselhWVW1tw2JC92hYbhbLdOKWlMys40w=w357)
...
Some rich result types have a two-stage search result: the initial search result is a short list containing the top results, with minimal information and a link to a detailed view of each item ( _list view_); clicking a list item opens a detailed view of the selected item ( _details view_), which contains links to one or more providers for that item.
...
- [Job posting](https://developers.google.com/search/docs/data-types/job-postings)
...
- **Click:** Clicking an item in list view (in the short or full list) counts as a click for the provider shown in the list item. Expanding the short list on desktop automatically opens the details view for the first item, but does _not_ count as a click on the first list item.
- **Impression:** An impression is counted when a list item is _**visible**_ in the short list view, and _**also**_ when the list view is expanded, whether or not the item is actually scrolled into view. Therefore, if an item is visible in the short view and the user then clicks to expand the list, **two** impressions are counted. If the item were not visible in the short view, and the user clicks to expand the list, only one impression is counted.
...
( _List container position_ (usually 1) + _position within the list_ (2)) / 2 = 1.5.
...
- **Impression:**All providers listed in the details view get an impression, no matter how the user got to the details view. A details view can be opened in the following ways:
  ...
  - Following a direct link to a details view (for example, someone shares the link with them). [This can cause your impressions to be higher than your clicks.](https://support.google.com/webmasters/answer/7042828?hl=en#impressions_higher_than_clicks)
...
**Why are my details _impressions_ higher than my list _clicks_?** There are a few reasons this can happen:
...
A [Media Action](https://developers.google.com/actions/media/) result enables a user to initiate playback of audio or video content directly from search results. (Podcast results are not counted as media action results.)
...
- **Position:** [Standard position rules](https://support.google.com/webmasters/answer/7042828?hl=en#position) apply.
...
Data from [news.google.com](https://news.google.com/topstories), and from the Google News app on [Android](https://play.google.com/store/apps/details?id=com.google.android.apps.magazines) and [iOS](https://apps.apple.com/us/app/google-news/id459182288) is shown in the [Performance report for News](https://support.google.com/webmasters/answer/10083653).

- **Click:** A click from [Google News](https://support.google.com/webmasters/answer/10083653?hl=en&ref_topic=9384513#covered_platforms) that landed the user on your property.
- **Impression:** How many links to your site a user saw on [Google News](https://support.google.com/webmasters/answer/10083653?hl=en&ref_topic=9384513#covered_platforms). Impressions are counted only when the link is scrolled into view. Scrolling away and scrolling back, or paging away then back, is counted as a single impression during a single session.
- **Position:** Not recorded.

## News tab in Search results

When viewing News results in the News tab in Google Search, results are loaded in pages, the same as [plain blue links](https://support.google.com/webmasters/answer/7042828?hl=en#old_blue).
...
- **Click**: The user clicks the main link
- **Impression:** Standard impression rules apply.
- **Position**: Standard position rules apply.

## Practice problem

A [practice problem result](https://developers.google.com/search/docs/advanced/structured-data/practice-problems) is a practice problem that users can answer on the search page that contains a link to the source site.
...
[Post to the help community  Get answers from community members](https://support.google.com/webmasters/community?hl=en&help_center_link=CIzurQMSK1doYXQgYXJlIGltcHJlc3Npb25zLCBwb3NpdGlvbiwgYW5kIGNsaWNrcz8)
...
[Help Center](https://support.google.com/webmasters/?hl=en) [Community](https://support.google.com/webmasters/community?hl=en&help_center_link=CIzurQMSK1doYXQgYXJlIGltcHJlc3Npb25zLCBwb3NpdGlvbiwgYW5kIGNsaWNrcz8) [Search Console](https://search.google.com/search-console)
...
By continuing, you agree Google uses your answers, [account & system info](https://support.google.com/webmasters/answer/7042828?hl=en#) to improve services, per our [Privacy](https://myaccount.google.com/privacypolicy?hl=en) & [Terms](https://policies.google.com/terms?hl=en).
